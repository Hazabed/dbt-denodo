...
A [dbt](https://www.getdbt.com/) adapter for [Denodo Platform 8](https://community.denodo.com/docs/html/browse/8.0/en/) (Virtual DataPort).
...
> suite; the functional suite requires a live VDP server. See
...
- Denodo Platform 8 with the VDP server's ODBC (PostgreSQL-compatible) interface enabled
  (port 9996 by default)
...
for Denodo 8. This interface accepts VQL, so the adapter issues native VQL statements.
...
throughput but only exists in **Denodo 9.1+**, so it is not an option for Platform 8.
...
These follow documented Denodo behavior but could not be verified end-to-end without a
live VDP 8 instance. Each is isolated in one macro for easy patching:
...
pytest tests/functional           # needs a live Denodo VDP 8 server
```
...
If you have access to a live Denodo VDP 8 server, running the functional
